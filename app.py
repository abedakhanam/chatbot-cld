...
def rewrite_query(user_text: str) -> str:
    ...
    vague_patterns = ['how to do it', 'explain', 'tell me more', 'steps']
    is_followup = any(p in t.lower() for p in vague_patterns) or len(t.split()) < 5
    
    if is_followup and st.session_state.messages:
        prev_q = [m['content'] for m in st.session_state.messages if m['role'] == 'user']
        if prev_q:
            return f"{prev_q[-1]} - {t}"  # Anchor to previous question
    
    # Original pronoun expansion logic
    ...
    if any((" " + p + " ") in (" " + t.lower() + " ") for p in vague_pronouns):
        ...
        hint = ", ".join(desc) if desc else "previous topic"
        t = f"In context of {hint}, {t}"
...
def main():
    ...
    for m in st.session_state.messages:
        ...
        if m["role"] == "assistant" and m.get("citations"):
            citations = f"<div style='margin-top:12px; padding-top:8px; border-top:1px solid #ddd; font-size:0.85em; color:#666;'><strong>Sources:</strong><br/>{'<br/>'.join(['• ' + c for c in m['citations']])}</div>"
        # Escape/format minimal; Streamlit markdown handles basic safety—this is static HTML block
        content = m["content"]
        content = re.sub(r'\[([^\]]+?)\](?!\()', r'<sup style="color:#999; font-size:0.7em;">[\1]</sup>', content)
